...
router.get('/sync', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`[USAGE SYNC] GET for user ${userId}: presentations=${user.presentationsCreated}, academic=${user.academicWorksCreated}`);
        res.json({
            success: true,
            data: {
        ...
        logger_1.logger.error(`[USAGE SYNC] GET error: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
router.post('/sync', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`[USAGE SYNC] POST increments for user ${userId}: ${JSON.stringify(updateData)}`);
        ...
        logger_1.logger.error(`[USAGE SYNC] POST error: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
router.post('/increment', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`[USAGE INCREMENT] ${field}: ${currentValue} → ${newValue} for user ${userId}`);
        const updatedUser = await index_1.prisma.user.update({
            where: { id: userId },
            data: { [field]: newValue },
            select: { [field]: true },
        ...
        logger_1.logger.error(`[USAGE INCREMENT] error: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
router.post('/reset-daily', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`[USAGE RESET] Daily reset for user ${userId}`);
        ...
        logger_1.logger.error(`[USAGE RESET] error: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
router.post('/reset-monthly', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`[USAGE RESET] Monthly reset for user ${userId}`);
        await index_1.prisma.user.update({
            where: { id: userId },
            data: {
