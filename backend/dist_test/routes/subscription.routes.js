...
router.get('/status', async (req, res) => {
    try {
        ...
        logger_1.logger.error(`Get subscription status error: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
router.get('/usage', async (req, res) => {
    try {
        ...
        const usageLogs = await index_1.prisma.usageLog.findMany({
            where: {
                userId,
                createdAt: { gte: since },
            },
        ...
        logger_1.logger.error(`Get usage error: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
router.post('/check-limit', async (req, res) => {
    try {
        ...
        logger_1.logger.error(`Check limit error: ${error instanceof Error ? error.message : 'Unknown error'}`);
