...
router.post('/', [
    ...
    (0, express_validator_1.body)('type').optional().isIn(['DRAFT', 'OUTLINE', 'CHAPTER', 'ABSTRACT', 'CONCLUSION', 'FINAL'])
], async (req, res) => {
    try {
        ...
        let version = 1;
        if (parentId) {
            const parent = await index_1.prisma.document.findUnique({
                where: { id: parentId }
            });
            if (parent) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existingDoc = await index_1.prisma.document.findFirst({
            where: {
                id: req.params.id,
                userId: req.userId
            }
        ...
        if (createNewVersion) {
            ...
            document = await index_1.prisma.document.create({
                data: {
            ...
            // Update in place
            document = await index_1.prisma.document.update({
...
router.delete('/:id', async (req, res) => {
    try {
        const document = await index_1.prisma.document.deleteMany({
            where: {
                id: req.params.id,
                userId: req.userId
            }
...
router.get('/:id/history', async (req, res) => {
    try {
        ...
        const allVersions = await index_1.prisma.document.findMany({
            where: {
