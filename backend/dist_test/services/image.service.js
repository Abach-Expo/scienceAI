...
async function searchPexels(query) {
    if (!PEXELS_API_KEY)
        return null;
    try {
        const response = await fetch(`https://api.pexels.com/v1/search?query=${encodeURIComponent(query)}&per_page=15&orientation=landscape`, { headers: { Authorization: PEXELS_API_KEY } });
        if (!response.ok)
            return null;
        ...
        logger_1.logger.error('[Images] Pexels error:', e);
    }
    return null;
}
// Unsplash search
async function searchUnsplash(query) {
    if (!UNSPLASH_ACCESS_KEY)
        return null;
    try {
        const response = await fetch(`https://api.unsplash.com/search/photos?query=${encodeURIComponent(query)}&per_page=15&orientation=landscape`, { headers: { Authorization: `Client-ID ${UNSPLASH_ACCESS_KEY}` } });
        if (!response.ok)
            return null;
        const data = await response.json();
        ...
        logger_1.logger.error('[Images] Unsplash error:', e);
    }
    return null;
}
// Fallback photos
