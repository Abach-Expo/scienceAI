...
exports.TASK_CONFIGS = exports.SAFETY_CONSTRAINTS = exports.RESPONSE_FORMATS = exports.RESPONSE_STYLES = exports.SCIENCE_AI_IDENTITY = void 0;
exports.composeSystemPrompt = composeSystemPrompt;
exports.selectProvider = selectProvider;
// ==================== БАЗОВАЯ ЛИЧНОСТЬ ====================
exports.SCIENCE_AI_IDENTITY = `Ты — Science AI, продвинутый AI-ассистент для научной и академической работы.
...
exports.SAFETY_CONSTRAINTS = `ОГРАНИЧЕНИЯ:
...
function composeSystemPrompt(taskType, customSystemPrompt, options) {
    const config = exports.TASK_CONFIGS[taskType] || exports.TASK_CONFIGS.chat;
    const style = options?.styleOverride || config.style;
    const format = options?.formatOverride || config.format;
    const parts = [
        exports.SCIENCE_AI_IDENTITY,
        '',
        // Если есть кастомный промпт от пользователя/фронтенда — он дополняет, а не заменяет
        customSystemPrompt || config.systemPrompt,
        '',
        exports.RESPONSE_STYLES[style],
        '',
        exports.RESPONSE_FORMATS[format],
        '',
        exports.SAFETY_CONSTRAINTS,
    ];
    return parts.join('\n');
}
/**
 ...
 */
function selectProvider(taskType, availableProviders, preferredProvider) {
    const config = exports.TASK_CONFIGS[taskType] || exports.TASK_CONFIGS.chat;
    ...
    const preferred = config.provider === 'auto' ? getBestProvider(taskType) : config.provider;
...
function getBestProvider(taskType) {
    ...
    if (claudeTasks.includes(taskType))
        return 'anthropic';
    // Задачи где Grok хорош (быстрые ответы, свежие данные)
    const grokTasks = ['chat'];
    if (grokTasks.includes(taskType))
        return 'xai';
    // Остальное — OpenAI GPT-4o
    return 'openai';
}
function getDefaultModel(provider, taskType) {
    switch (provider) {
        case 'openai': {
...
//# sourceMappingURL=llmGateway.prompts.js.map