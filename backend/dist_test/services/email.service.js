...
class EmailService {
    ...
    initialize() {
        if (!nodemailer) {
            logger_1.logger.warn('nodemailer not installed. Run: npm install nodemailer @types/nodemailer');
            return;
        }
        ...
        if (SMTP_HOST && SMTP_USER && SMTP_PASS) {
            try {
                ...
                logger_1.logger.warn('Failed to configure email service, falling back to console logging');
            }
        }
        else {
            logger_1.logger.warn('Email service not configured (missing SMTP_HOST, SMTP_USER, SMTP_PASS). Emails will be logged to console.');
        }
    }
    async sendEmail(options) {
        ...
        if (this.isConfigured && this.transporter) {
            try {
                ...
                logger_1.logger.error(`Failed to send email to ${options.to}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                // Fall through to console logging
            }
        }
        // Fallback: log to console (useful for development)
        logger_1.logger.info(`[EMAIL FALLBACK] To: ${options.to}`);
        logger_1.logger.info(`[EMAIL FALLBACK] Subject: ${options.subject}`);
        logger_1.logger.info(`[EMAIL FALLBACK] Body: ${options.text || options.html}`);
        return false;
    }
    // ==========================================
    // TEMPLATE METHODS
    ...
    async sendSubscriptionConfirmation(email, planName, expiryDate) {
        return this.sendEmail({
            to: email,
