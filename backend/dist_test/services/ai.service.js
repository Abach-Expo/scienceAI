...
try {
    ...
    logger_1.logger.warn('Anthropic SDK not installed. Claude models will fallback to GPT-4o. Install with: npm i @anthropic-ai/sdk');
}
...
class AIService {
    ...
    constructor() {
        ...
        if (Anthropic && process.env.ANTHROPIC_API_KEY) {
            ...
            logger_1.logger.info('Anthropic SDK initialized — Claude Sonnet 4 active for text generation');
        }
        else {
            logger_1.logger.info('Anthropic not configured — using GPT-4o for all tasks');
        }
    }
    /**
     ...
     */
    async generate(taskType, systemPrompt, userPrompt, options = {}) {
        const routingKey = (taskType in MODEL_ROUTING ? taskType : 'chat');
        ...
        if (TEXT_TASKS.includes(routingKey)) {
            ...
            if (cleaned.length > 2000) {
                try {
                    ...
                    logger_1.logger.warn(`Humanization pass failed, returning cleaned version: ${err instanceof Error ? err.message : 'Unknown error'}`);
...
- Output ONLY the rewritten text. No preamble, no commentary, no explanation.`;
        ...
        if (result.length < text.length * 0.6) {
            logger_1.logger.warn(`Humanization output too short (${result.length} vs ${text.length}), using original`);
            return text;
        }
     ...
     */
    async generateWithRouting(taskType, systemPrompt, userPrompt, options = {}) {
        ...
        if (routing.provider === 'anthropic' && this.anthropic) {
            try {
                ...
                logger_1.logger.warn(`Claude error, falling back to GPT-4o: ${error instanceof Error ? error.message : 'Unknown error'}`);
                // Фолбэк на OpenAI
            }
        }
     ...
     */
    async generateStream(taskType, systemPrompt, userPrompt, options = {}, onChunk) {
        const routingKey = (taskType in MODEL_ROUTING ? taskType : 'chat');
        const routing = MODEL_ROUTING[routingKey];
        ...
        if (routing.provider === 'anthropic' && this.anthropic) {
            try {
                ...
                if (!accumulated)
                    throw new Error('No content from Claude stream');
                ...
                logger_1.logger.warn(`Claude streaming error, falling back to OpenAI: ${error instanceof Error ? error.message : 'Unknown error'}`);
                accumulated = '';
            }
        }
        ...
        if (!accumulated)
            throw new Error('No content from OpenAI stream');
        if (TEXT_TASKS.includes(routingKey)) {
            accumulated = this.postProcessHumanize(accumulated);
        }
        return { content: accumulated, model, provider: routing.provider === 'anthropic' ? 'openai' : routing.provider };
    }
    async generateOutline(topic, projectType, additionalContext) {
...
Return as JSON array with structure: [{ "title": string, "description": string, "subsections": [], "estimatedWords": number }]`;
        try {
            ...
            logger_1.logger.error('Error generating outline:', error);
            throw error;
        }
    }
    async generateArguments(topic, outline, researchQuestions) {
...
Return as structured JSON.`;
        try {
            ...
            logger_1.logger.error('Error generating arguments:', error);
            throw error;
        }
    }
    async generateDraft(title, outline, specificSection, references, customInstructions, targetWords) {
...
Remember: vary your sentence lengths dramatically, take clear positions, use specific data and examples, and write with genuine academic personality. The text must pass AI detection tools as human-written.`;
        try {
            ...
            logger_1.logger.error('Error generating draft:', error);
            throw error;
        }
    }
...
Return as structured JSON.`;
        try {
            ...
            const content = await this.generateWithRouting(taskType, systemPrompt, userPrompt, {
                temperature: 0.3,
                maxTokens: 4000,
                jsonMode: true,
            });
    ...
    async improveStyle(content, instructions) {
...
Return as JSON with structure: { "content": string, "changes": [{ "original": string, "improved": string, "reason": string }] }`;
        try {
            ...
            logger_1.logger.error('Error improving style:', error);
            throw error;
        }
    }
    async selfReview(content, projectType) {
...
Return as JSON with structure:
{
  ...
  "revisedSections": [{ "original": string, "revised": string, "explanation": string }]
        ...
        try {
            ...
            logger_1.logger.error('Error performing self-review:', error);
            throw error;
        }
    }
}
