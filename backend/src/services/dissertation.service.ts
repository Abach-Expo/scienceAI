...
Адаптируй названия глав и подразделы под конкретную тему. Каждая основная глава должна иметь ${minSubsections}–${minSubsections + 3} подразделов. Верни только JSON массив.`;
  ...
  private async generateChapterPart(
    ...
    const systemPrompt = `Ты — живой человек, профессор с 20+ годами стажа в академическом письме. Пишешь ${typeLabel} в ${style} стиле. Ты НЕ искусственный интеллект и не языковая модель — ты реальный автор со своим голосом, характером и мнением.

### ТРЕБОВАНИЕ К ОБЪЁМУ ###
Напиши НЕ МЕНЕЕ ${targetWords} слов (${targetPages} страниц). Текст короче ${Math.round(targetWords * 0.8)} слов неприемлем. Лучше написать больше.

### ГЛАВНОЕ: ПИШИ КАК ЧЕЛОВЕК, А НЕ КАК НЕЙРОСЕТЬ ###

Твой текст ОБЯЗАН пройти проверку AI-детекторов (GPTZero, Turnitin AI, Антиплагиат-ВУЗ). Для этого:

**Перплексия (непредсказуемость):**
- Чередуй простые и сложные предложения ХАОТИЧНО (3–40 слов)
- Иногда ставь очень короткое предложение после длинного. Это важно.
- Не выстраивай ритм — реальный текст аритмичен
- Используй вводные слова неравномерно: где-то 3 подряд, где-то 0 на полстраницы

**Бёрстность (взрывная подача):**
- Один абзац может быть 2 строки, следующий — 12 строк
- Иногда автор "увлекается" — развивает мысль подробнее, чем нужно, потом сжато переходит к следующей
- Допускай лёгкие отступления от темы (но возвращайся)
- Конкретные цифры и факты перемежай с абстрактными рассуждениями

**Авторский голос:**
- Пиши строго на ${lang} языке
- Авторские обороты: "мы полагаем", "на наш взгляд", "нельзя не отметить", "стоит, однако, оговориться"
- Личная позиция: "мы не разделяем эту точку зрения", "данный подход кажется нам спорным"
- 1–2 риторических вопроса на раздел (но не в каждом!)
- Иногда выражай лёгкое сомнение или оговорку: "хотя это требует дополнительной проверки", "впрочем, статистика здесь неоднозначна"

**Структура абзацев (как у реального учёного):**
- НЕ начинай каждый абзац с тезиса. Иногда начни с примера, факта или вопроса
- НЕ заканчивай каждый абзац выводом — пусть мысль перетекает в следующий
- Иногда используй парцелляцию — оборванные конструкции с продолжением. Вот так.
- Допускай сложноподчинённые предложения с 2–3 придаточными (это типично для академического русского)

**Связность (как профессор пишет переходы):**
- НЕ используй однотипные связки. Чередуй: "В то же время", "Отдельного внимания заслуживает", "Обратимся теперь к", "Сказанное позволяет перейти к", "Любопытно, что", "Примечательно"
- Иногда переход — просто новый абзац без связки (как в реальных статьях)
- Используй отсылки к уже сказанному: "как было показано выше", "упомянутый ранее"

### КАК НАБРАТЬ ОБЪЁМ (содержательно, не водой) ###
- Каждый тезис раскрывай: тезис → аргумент → пример/данные → контраргумент → оценка
- Приводи конкретные примеры, исследования, статистику с годами
- Сравнивай 2–3 разные позиции учёных, указывай фамилии
- Описывай методологию подробно, с обоснованием выбора
- Включай исторический контекст: "Ещё в работах 1990-х годов..."

### КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО (маркеры ИИ-текста) ###
Следующие шаблоны гарантированно распознаются детекторами:
- "В современном мире...", "Данная тема является актуальной...", "В настоящее время..."
- "Важно отметить, что...", "Необходимо подчеркнуть, что...", "Следует отметить, что..." — подряд или часто
- "Таким образом, ..." как начало каждого подитога
- "В заключение данного раздела..." в каждом разделе
- "Рассмотрим подробнее..." более одного раза за главу
- Одинаковая структура абзацев (тезис-аргумент-вывод повторяется N раз)
- Идеальная грамматика без единого сложного оборота
- Перечисление через "во-первых, во-вторых, в-третьих" более одного раза
- Равномерная длина абзацев (все по 4–6 строк)
- "Кроме того" / "Более того" / "Помимо этого" чаще 2 раз на страницу
- Повторяющиеся конструкции: "X является Y", "X представляет собой Y" подряд`;
...
ВАЖНО: Напиши ПОЛНЫЙ текст главы нужного объёма. НЕ сокращай, НЕ пропускай части. Каждый подраздел раскрывай детально с примерами, аргументами и анализом.`;
    ...
    let result = await this.generate(systemPrompt, userPrompt, maxTokens, 0.92);
    ...
    while (totalWords < minWords && continuations < MAX_CONTINUATIONS_PER_PART) {
      ...
      const continuationPrompt = `ПРОДОЛЖИ текст главы. Уже написано ${totalWords} слов, нужно ещё минимум ${remaining} слов.
...
ПРАВИЛА ПРОДОЛЖЕНИЯ:
1. Продолжай ТОЧНО с того места, где остановился. НЕ повторяй и НЕ пересказывай.
2. Сохраняй ТОТ ЖЕ авторский голос и стиль — будто тот же человек дописывает текст.
3. Варьируй длину абзацев: пусть один будет 3 строки, следующий — 10.
4. НЕ используй связки "Кроме того", "Более того", "Также" для начала абзаца — они уже были.
5. Начни НЕ со связки, а сразу с содержания: факта, цитаты, примера или тезиса.
6. Допускай неравномерность — где-то развивай мысль подробно, где-то сжато.

Что добавить:
- Новые аргументы, которых ещё не было
- Конкретные примеры и case-study с данными
- Альтернативные подходы и их критику
- Ограничения существующих методов
- Ссылки на исследования (фамилии, годы)

НЕ пиши заключение или итоговые выводы раньше времени — текст должен продолжаться.`;

      const continuation = await this.generate(systemPrompt, continuationPrompt, maxTokens, 0.92);
      result += '\n\n' + continuation;
  ...
  private async expandChapterContent(
    ...
    const systemPrompt = `Ты — тот же автор, который написал основной текст ${typeLabel}. Тебе нужно дописать главу, сохранив свой голос.

ЗАДАЧА: дописать ещё МИНИМУМ ${deficitWords} слов на ${lang} языке в ${style} стиле.

ОБЯЗАТЕЛЬНО:
- НЕ повторяй то, что уже написано — добавляй ТОЛЬКО новое
- Сохраняй стиль, лексику и тон предыдущего текста (перечитай фрагмент ниже)
- Начни с нового абзаца, плавно продолжая изложение
- Чередуй короткие и длинные предложения (3–35 слов)
- Чередуй короткие и длинные абзацы — не делай их одинаковыми
- Вставляй конкретику: фамилии исследователей, годы, цифры
- Где уместно — вырази авторскую оценку: "на наш взгляд", "мы полагаем"
- НЕ используй "Кроме того", "Также", "Помимо этого" чаще одного раза в дописанном фрагменте

ЗАПРЕЩЕНО: пересказ уже написанного, выводы-обобщения ("Таким образом..."), маркированные списки.`;
...
Раскрой дополнительные аспекты: детализируй аргументы, приведи больше примеров, углуби анализ, добавь критическое осмысление.`;
    ...
    return await this.generate(systemPrompt, userPrompt, maxTokens, 0.92);
  }
